...
clearInterval(this.s.scrollInterval);this.s.scrollInterval=null;var f=this.s.start.nodes,g=e.unique(b.rows({page:"current"}).nodes().toArray()),j={},i=[],k=[],l=this.s.getDataFn,o=this.s.setDataFn;a=0;for(d=f.length;a<d;a++)if(f[a]!==g[a]){var m=b.row(g[a]).id(),p=b.row(g[a]).data(),n=b.row(f[a]).data();m&&(j[m]=l(n));i.push({node:g[a],oldData:l(p),newData:l(n),newPosition:a,oldPosition:e.inArray(g[a],f)});k.push(g[a])}this._emitEvent("row-reorder",[i,{dataSrc:c,nodes:k,values:j}]);this.c.editor&&
this.c.editor.edit(k,!1,{submit:"changed"}).multiSet(c,j).submit();if(this.c.update){a=0;for(d=i.length;a<d;a++)f=b.row(i[a].node).data(),o(f,i[a].newData),b.columns().every(function(){this.dataSrc()===c&&b.cell(i[a].node,this.index()).invalidate("data")});b.draw(!1)}}});g.defaults={dataSrc:0,editor:null,selector:"td:first-child",snapX:!1,update:!0};g.version="1.1.0-dev";e.fn.dataTable.RowReorder=g;e.fn.DataTable.RowReorder=g;e(document).on("init.dt.dtr",function(b,a){if("dt"===b.namespace){var d=
a.oInit.rowReorder,c=f.defaults.rowReorder;if(d||c)c=e.extend({},d,c),!1!==d&&new g(a,c)}});return g});
