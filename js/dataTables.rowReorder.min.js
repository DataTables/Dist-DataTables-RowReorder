...
d;this.s.bodyTop=e(b.table().body()).offset().top;this.s.windowHeight=e(window).height()},_clone:function(b){var a=e(this.s.dt.table().node().cloneNode(!1)).addClass("dt-rowReorder-float").append("<tbody/>").append(b.clone(!1)),d=b.outerWidth(),c=b.outerHeight(),h=b.children().map(function(){return e(this).width()});a.width(d).height(c).find("tr").children().each(function(a){this.style.width=h[a]+"px"});a.appendTo("body");this.dom.clone=a},_clonePosition:function(b){var a=this.s.start,d=this._eventToPage(b,
"Y")-a.top,b=this._eventToPage(b,"X")-a.left,c=this.c.snapX;this.dom.clone.css({top:d+a.offsetTop,left:!0===c?a.offsetLeft:"number"===typeof c?a.offsetLeft+c:b+a.offsetLeft})},_emitEvent:function(b,a){this.s.dt.iterator("table",function(d){e(d.nTable).triggerHandler(b+".dt",a)})},_eventToPage:function(b,a){return-1!==b.type.indexOf("touch")?b.originalEvent.touches[0]["page"+a]:b["page"+a]},_mouseDown:function(b,a){var d=this,c=this.s.dt,h=this.s.start,f=a.offset();h.top=this._eventToPage(b,"Y");h.left=
this._eventToPage(b,"X");h.offsetTop=f.top;h.offsetLeft=f.left;h.nodes=e.unique(c.rows({page:"current"}).nodes().toArray());this._cachePositions();this._clone(a);this._clonePosition(b);this.dom.target=a;a.addClass("dt-rowReorder-moving");e(document).on("mouseup.rowReorder touchend.rowReorder",function(a){d._mouseUp(a)}).on("mousemove.rowReorder touchmove.rowReorder",function(a){d._mouseMove(a)});e(window).width()===e(document).width()&&e(document.body).addClass("dt-rowReorder-noOverflow")},_mouseMove:function(b){this._clonePosition(b);
for(var a=this._eventToPage(b,"Y")-this.s.bodyTop,d=this.s.middles,c=null,h=this.s.dt,f=h.table().body(),i=0,g=d.length;i<g;i++)if(a<d[i]){c=i;break}null===c&&(c=d.length);if(null===this.s.lastInsert||this.s.lastInsert!==c)0===c?this.dom.target.prependTo(f):(a=e.unique(h.rows({page:"current"}).nodes().toArray()),c>this.s.lastInsert?this.dom.target.before(a[c-1]):this.dom.target.after(a[c])),this._cachePositions(),this.s.lastInsert=c;b=this._eventToPage(b,"Y")-document.body.scrollTop;c=this.s.scrollInterval;
...
clearInterval(this.s.scrollInterval);this.s.scrollInterval=null;var f=this.s.start.nodes,g=e.unique(b.rows({page:"current"}).nodes().toArray()),i={},j=[],k=[],l=this.s.getDataFn,o=this.s.setDataFn;a=0;for(d=f.length;a<d;a++)if(f[a]!==g[a]){var m=b.row(g[a]).id(),p=b.row(g[a]).data(),n=b.row(f[a]).data();m&&(i[m]=l(n));j.push({node:g[a],oldData:l(p),newData:l(n),newPosition:a,oldPosition:e.inArray(g[a],f)});k.push(g[a])}f=[j,{dataSrc:c,nodes:k,values:i}];this._emitEvent("row-reorder",f);this.c.editor&&
this.c.editor.edit(k,!1,{submit:"changed"}).multiSet(c,i).submit();if(this.c.update){a=0;for(d=j.length;a<d;a++)i=b.row(j[a].node).data(),o(i,j[a].newData),b.columns().every(function(){this.dataSrc()===c&&b.cell(j[a].node,this.index()).invalidate("data")});this._emitEvent("row-reordered",f);b.draw(!1)}}});g.defaults={dataSrc:0,editor:null,selector:"td:first-child",snapX:!1,update:!0};g.version="1.1.0-dev";e.fn.dataTable.RowReorder=g;e.fn.DataTable.RowReorder=g;e(document).on("init.dt.dtr",function(b,
a){if("dt"===b.namespace){var d=a.oInit.rowReorder,c=f.defaults.rowReorder;if(d||c)c=e.extend({},d,c),!1!==d&&new g(a,c)}});return g});
